...
//					userManager.savaDeviceMessage(user.getUserId(), serial, deviceType);
    ...
    public JSONMessage sendMessage(@RequestParam(defaultValue = "") String jid,
        ...
        logger.info("[HTTP消息发送入口] 接收到发送消息请求 - 用户ID: {}, 目标JID: {}, 聊天类型: {}, 消息类型: {}, 内容长度: {}, 文件名: {}, 文件大小: {}", 
                   userId, jid, chatType, type, content != null ? content.length() : 0, fileName, fileSize);
        
        if (StringUtil.isEmpty(jid) || StringUtil.isEmpty(content)) {
            logger.warn("[HTTP消息发送入口] 参数验证失败 - 用户ID: {}, JID为空: {}, 内容为空: {}", 
                       userId, StringUtil.isEmpty(jid), StringUtil.isEmpty(content));
            return JSONMessage.success();
        }
        
        try {
            logger.info("[HTTP消息发送入口] 开始调用UserManager.sendMessage - 用户ID: {}, 目标JID: {}", userId, jid);
            userManager.sendMessage(jid, chatType, type, content, fileName, fileSize);
            logger.info("[HTTP消息发送入口] UserManager.sendMessage调用成功 - 用户ID: {}, 目标JID: {}", userId, jid);
        } catch (Exception e) {
            logger.error("[HTTP消息发送入口] UserManager.sendMessage调用异常 - 用户ID: {}, 目标JID: {}, 异常信息: {}", 
                        userId, jid, e.getMessage(), e);
        }
